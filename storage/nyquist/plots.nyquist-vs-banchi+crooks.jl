...
let
	...
	theplot = plot( ∂f , plotrange
		; 	label="",
			xaxis="𝑥", xlimits=(a,b+σ/4),
			yaxis="black: 𝑓(𝑥), blue: 𝑓'(𝑥)",
			color=:blue)
...
let
	...
	scatter!(theplot, err , plotrange
		; 	label="",
			xaxis="𝑥", xlimits=(a,b+σ/4),
			yaxis="absolute error, ε=$(BC_PLOT[4])e$(-BC_PLOT[5])",
#			ylimits=[-0.1,+0.1],
			markersize=0.5,
			markercolor=:green,
...
let ε = 10.0^-MAKE_DATA_INPUT[1]
	...
	if RELATIVE
		theplot = plot(;
				...
				yaxis=yaxistext*" differences: median",
				yticks=vcat([j/100 for j= -20:1:20],[j for j=-100.0:10.0:+100]))
	else
		theplot = plot(;
				xaxis="θ",
				xticks=[0.1j/8ε for j=0:10],
...
julia_version = "1.9.1"
manifest_format = "2.0"
project_hash = "6a987f46c823822e52c96cb5af715d9f048c49a2"
